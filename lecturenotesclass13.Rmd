...
Finding the mean would be an important and valuable point estimate to use. 
...
They are single values and not representative of an entire dataset. Also, the
mean does not represent the middle (median), but rather the average. It is
important to understand many different point estimates when analyzing data
in order to grasp the full extent of significance of the data. 
These values are estimates and all have margins for error, so basing 
important decisions from point estimates may not be a good idea. 
...
I would expect there to be anywhere from 2-7 left handed students in any random
sample of 50 students. I would not be surprised if 3 were left handed, however
I would be very confused if 40 were left handed. 
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-alexandermcmullen/manhattan.csv")
```
...
(Please ignore impossible values.)
...
**Interpretation**: This means that we can be 95% confident that if the value is 
within the range (confidence interval), then it reflects the actual population 
mean rent.
...
boot_medians <- manhattan %>% 
  ...
  summarize(lb = quantile(stat, 0.025), ub = quantile(stat, 0.975))
```

The lower and upper bounds of the confidence interval are the limits of
the range of possible median population rent values within 95% confidence. 
This means we can be 95% confident that the actual median rent prices across
the population are somewhere in between $2162 and $2875.
